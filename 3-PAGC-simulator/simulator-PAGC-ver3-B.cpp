...
// - In addition to the garbage value, we _COULD_ have a list of all
...
//      NOTE: This _DOESN'T_ seem implemented. 2017-july-28 RLV
...
// Design 1:  -- No sub function call yet.
//  MAP:
//      func -> record {
...
//         maximum garbage
//      }
typedef std::vector< unsigned int > GarbageVector_t;
struct FunctionRec_t {
    public:
        ...
        {
            return FunctionRec_t::gvec2string( this->garbage_vector );
        }

        static string gvec2string( const GarbageVector_t &myvec )
        {
            string result;
        ...
        GarbageVector_t::iterator get_gvec_begin()
        {
            return this->garbage_vector.begin();
        }

        GarbageVector_t::iterator get_gvec_end()
        {
            return this->garbage_vector.end();
        }
    ...
    private:
        ...
        GarbageVector_t garbage_vector;
};
...
struct CNode_t {
    public:
        ...
        unsigned int get_method_id() const
        {
                       ...
                       MethodDeque &new_path_to_node )
        {
            ...
            auto iter = this->subtree.find( new_id );
            if (iter != subtree.end()) {
                // In there already
        ...
        CNode_t * find_path( MethodDeque &path )
        {
            ...
            if ( (path.size() == 0) ||
                ...
                assert( path[0]->getId() == this->method_id );
                return this;
            } else {
                // So path[0]->getId() == this->method_id,
                assert( path[0]->getId() == this->method_id );
                ...
                auto iter = this->subtree.find( new_id );
                if (iter != this->subtree.end()) {
                    // Found it, so no need to add 'new_id/method'
                    auto path_iter = path.begin();
                    path_iter++; // Go to path[1]
                    MethodDeque subpath( path_iter, path.end() ); // inclusive of beginning
                    return iter->second->find_path( subpath ); // Recursively find the rest of the path
                } else {
                    // Not found.
                    return NULL;
                }
        ...
        {
            // Preconditions:
            // - The top of the path should be at the end (size-1), not the
            //   beginning (0).
            ...
            // NOTE: We return the top node (ie the bottom of the path)
            // DEBUG ONLY: cout << "XXX: len path: " << path.size() << endl;
            if (path.size() == 0) {
                // Trivial task of add no path:
                return this;
            } else {
                Method *method = path.back(); // top of the path
                        ...
                        ofstream &funcout )
{
    ...
    // Map from method pointer to  total garbage.
    std::map< Method *, std::vector< unsigned int > > meth2garbage;
    ...
    while (queue.size() > 0) {
        ...
        for ( auto iter = current->begin_adjacent();
            ...
            if (fit == vend) {
                ...
                // TODO: Used to be path_id. Now replacing it with method id.
                //       But now, we need to aggregate on method id as the same method 
                //       may be found in different contexts even in a static call graph.
                // TODO: // Print out here. Output the record:
                // TODO: //   path_id, total_garbage, minimum, maximum, number_times
                // END TODO.
                // Aggregate according to method id.
                ...
                MethodId_t method_id = cptr->get_method_id();
                Method *method_ptr = ClassInfo::TheMethods[method_id];
                auto m2g_it = meth2garbage.find( method_ptr );
                if ( m2g_it ==  meth2garbage.end() ) {
                    GarbageVector_t tmpvec;
                    meth2garbage[method_ptr] = tmpvec;
                }
                meth2garbage[method_ptr].insert( m2g_it->second.end(),
                                                 rec.get_gvec_begin(),
                                                 rec.get_gvec_end() );
                // TODO TODO TODO: Is the folowing still needed?
                ...
                // TODO: Removed for now. 2017-0728 - RLV
                // TODO: string glist_str = rec.gvec2string();
                // TODO: // TODO: What about path_id? TODO
                // TODO: funcout << method_id << ","
                // TODO:         << total_garbage << "," << minimum << "," << maximum << ","
                // TODO:         << number << "," << glist_str
                // TODO:         << endl;
            }
        }
    }

    for ( auto iter = meth2garbage.begin();
          iter != meth2garbage.end();
          iter++ ) {
        Method *mptr = iter->first;
        GarbageVector_t gvec = iter->second;
        MethodId_t method_id = mptr->getId();
        string method_name = mptr->getName();
        if (gvec.size() > 0) {
            string glist_str = FunctionRec_t::gvec2string(gvec);
            funcout << method_id << "," << method_name << ","
                    << glist_str
                    << endl;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    funcout << "\"method_id\",\"method_name\",\"garbage_list\"" << endl;

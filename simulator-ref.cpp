...
{
    if (argc != 7) {
        cout << "Usage: " << argv[0] << " <namesfile> <output base name> <main.class> <main.function>" << endl;
    ...
    string main_class(argv[3]);
    string main_function(argv[4]);
    cout << "Main class: " << main_class << endl;
    cout << "Main function: " << main_function << endl;

    // Output files
    // MAYBE: string cycle_filename( basename + "-CYCLES.csv" );
    // MAYBE: string objectinfo_filename( basename + "-OBJECTINFO.txt" );
    // MAYBE: string edgeinfo_filename( basename + "-EDGEINFO.txt" );
    // MAYBE: string summary_filename( basename + "-SUMMARY.csv" );

    // NOT NEEDED? SAVED ALREADY: string reference_summary_filename( basename + "-REF-SUMMARY.csv" );
    // NOT NEEDED? SAVED ALREADY: string ref_reverse_summary_filename( basename + "-REF-REVERSE-SUMMARY.csv" );
    // NOT NEEDED? SAVED ALREADY: string stability_summary_filename( basename + "-STABILITY-SUMMARY.csv" );

    cout << "Read names file..." << endl;
    ClassInfo::read_names_file( argv[1], // names filename
                                main_class,
                                main_function );

    cout << "Start trace..." << endl;
    FILE* f = fdopen(0, "r");
    // MAYBE: ofstream edge_info_file(edgeinfo_filename);
    // MAYBE: edge_info_file << "---------------[ EDGE INFO ]----------------------------------------------------" << endl;
    ...
    Heap.end_of_program_nosave( final_time );

    // IDEA:
    // 1. Take all dead objects that died with a RC > 0.
    // 2. These are all in cycles.
    // 3. Sticky edges? references?
    // 4. Stable edges are sticky. Not all sticky edges are stable.
    // 5. Edges that die with the target object are sticky.
    if (false) {
        ...
        // MAYBE: // Output all objects info
        // MAYBE: output_all_objects2( objectinfo_filename,
        // MAYBE:                      Heap,
        // MAYBE:                      dag_keys,
        // MAYBE:                      dag_all_set,
        // MAYBE:                      all_keys,
        // MAYBE:                      final_time );
        //
        // MAYBE: output_reference_summary( reference_summary_filename,
        // MAYBE:                           ref_reverse_summary_filename,
        // MAYBE:                           stability_summary_filename,
        // MAYBE:                           edge_summary,
        // MAYBE:                           obj2ref_map,
        // MAYBE:                           stability_summary );
        // TODO: What next? 
        // Output cycles
        // MAYBE: set<int> node_set;
        // MAYBE: output_cycles( keyset,
        // MAYBE:                cycle_filename,
        // MAYBE:                node_set );
    }

    // PROBRABLY NOT: ofstream summary_file(summary_filename);
    // PROBRABLY NOT: summary_file << "---------------[ SUMMARY INFO ]----------------------------------------------------" << endl;
    // PROBRABLY NOT: summary_file << "number_of_objects," << Heap.size() << endl
    // PROBRABLY NOT:              << "number_of_edges," << Heap.numberEdges() << endl
    // PROBRABLY NOT:              << "died_by_stack," << Heap.getTotalDiedByStack2() << endl
    // PROBRABLY NOT:              << "died_by_heap," << Heap.getTotalDiedByHeap2() << endl
    // PROBRABLY NOT:              << "died_by_global," << Heap.getTotalDiedByGlobal() << endl
    // PROBRABLY NOT:              << "died_at_end," << Heap.getTotalDiedAtEnd() << endl
    // PROBRABLY NOT:              << "last_update_null," << Heap.getTotalLastUpdateNull() << endl
    // PROBRABLY NOT:              << "last_update_null_heap," << Heap.getTotalLastUpdateNullHeap() << endl
    // PROBRABLY NOT:              << "last_update_null_stack," << Heap.getTotalLastUpdateNullStack() << endl
    // PROBRABLY NOT:              << "last_update_null_size," << Heap.getSizeLastUpdateNull() << endl
    // PROBRABLY NOT:              << "last_update_null_heap_size," << Heap.getSizeLastUpdateNullHeap() << endl
    // PROBRABLY NOT:              << "last_update_null_stack_size," << Heap.getSizeLastUpdateNullStack() << endl
    // PROBRABLY NOT:              << "died_by_stack_only," << Heap.getDiedByStackOnly() << endl
    // PROBRABLY NOT:              << "died_by_stack_after_heap," << Heap.getDiedByStackAfterHeap() << endl
    // PROBRABLY NOT:              << "died_by_stack_only_size," << Heap.getSizeDiedByStackOnly() << endl
    // PROBRABLY NOT:              << "died_by_stack_after_heap_size," << Heap.getSizeDiedByStackAfterHeap() << endl
    // PROBRABLY NOT:              << "no_death_sites," << Heap.getNumberNoDeathSites() << endl
    // PROBRABLY NOT:              << "size_died_by_stack," << Heap.getSizeDiedByStack() << endl
    // PROBRABLY NOT:              << "size_died_by_heap," << Heap.getSizeDiedByHeap() << endl
    // PROBRABLY NOT:              << "size_died_at_end," << Heap.getSizeDiedAtEnd() << endl
    // PROBRABLY NOT:              << "vm_RC_zero," << Heap.getVMObjectsRefCountZero() << endl
    // PROBRABLY NOT:              << "vm_RC_positive," << Heap.getVMObjectsRefCountPositive() << endl
    // PROBRABLY NOT:              << "max_live_size," << Heap.maxLiveSize() << endl
    // PROBRABLY NOT:              << "main_func_uptime," << Exec.get_main_func_uptime() << endl
    // PROBRABLY NOT:              << "main_func_alloctime," << Exec.get_main_func_alloctime() << endl
    // PROBRABLY NOT:              << "final_time," << final_time << endl
    // PROBRABLY NOT:              << "final_time_alloc," << final_time_alloc << endl;
    // PROBRABLY NOT: summary_file << "---------------[ SUMMARY INFO END ]------------------------------------------------" << endl;
    // PROBRABLY NOT: summary_file.close();
    //---------------------------------------------------------------------
    cout << "---------------[ DONE ]------------------------------------------------------------" << endl;
    cout << "#     git version: " <<  build_git_sha << endl;
    cout << "#     build date : " <<  build_git_time << endl;
